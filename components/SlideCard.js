    ...
    return (
        <div onClick={() => setShowDetail(false)}
            ...
            <div onClick={e => e.stopPropagation()}
                 className={'h-[60vh] w-2/3 bg-black/85 border-white/20 border p-10 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg text-white flex flex-col gap-8  '}>
                <p className={' text-3xl font-bold '}> {projectInit[num - 1].title}</p>
                <div className={'flex flex-col gap-2'}>
                    <p className={'text-xl font-bold'}> {projectInit[num - 1].contents}</p>
                    <p className={'text-lg '}>{projectInit[num - 1].skills}</p>
